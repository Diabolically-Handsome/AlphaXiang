...
OUT="/home/laure/alphaxiang/training_runs/run_041b_v14c_cnn_policy_pairwise_from031a19000"
...
exec "$PY" -u xiangqi_train.py \
  ...
  --selfplay-dir-sampling-ratios 0.45 0.10 0.05 0.14 0.13 0.13 \
  --device cuda:0 \
  ...
  --warmup-steps 0 \
  --max-steps 19250 \
  --save-interval-steps 125 \
  --snapshot-interval-steps 125 \
  --eval-interval-steps 500 \
  --log-interval-steps 20 \
  --train-only-cnn-policy-residual-adapter \
  --policy-loss-weight 0.15 \
  --teacher-q-pairwise-loss-weight 0.40 \
  --teacher-q-pairwise-margin-logit 0.18 \
  --teacher-q-pairwise-min-gap-cp 80 \
  --teacher-q-pairwise-beta 1.0 \
  --teacher-q-pairwise-use-anchor-reference \
  --teacher-q-pairwise-bad-move-only \
  --bad-move-suppression-loss-weight 0.15 \
  --bad-move-suppression-margin-logit 0.25 \
  --bad-move-suppression-min-gap-cp 80 \
  --bad-move-suppression-beta 2.0 \
  --anchor-policy-kl-weight 4.0 \
