#!/usr/bin/env bash
# Trajectory probe (GPU1, no interference with the GPU0 trainer): probe the CURRENT latest
# run_058 snapshot vs Pika d1+noise / d2 / d3, 30 games each. Baselines from last night:
#   Stage-1:        d1n15=78.3%  d2=6.7%
#   d20 step187000: d1n15=100%   d2=23.3%
set -uo pipefail
...
mkdir -p "$OUT"
echo "[traj] probing $CKPT ($STEP) at $(date +%H:%M)"
...
run 2 0  71002
run 3 0  71003
echo "=== TRAJ $STEP ($(date +%H:%M)) ==="
for dd in d1n15 d2n0 d3n0; do echo "$dd: $(grep score_rate "$OUT/$dd/run.log" 2>/dev/null | tail -1)"; done
