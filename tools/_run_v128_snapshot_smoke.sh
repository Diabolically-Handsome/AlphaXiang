#!/bin/bash
# v12.8 candidate smoke: Pika d3/d4/d5 without symbolic guard.
...
run_external "pika_d3" 3 "$D3_DEVICE" 128303 &
PID3=$!
run_external "pika_d4" 4 "$D4_DEVICE" 128304 &
PID4=$!
STATUS=0
wait "$PID3" || STATUS=$?
wait "$PID4" || STATUS=$?
if [ "$STATUS" -ne 0 ]; then
    echo "v12.8 d3/d4 smoke failed with status=$STATUS" >&2
    exit "$STATUS"
fi

run_external "pika_d5" 5 "$D5_DEVICE" 128305
...
echo "v12.8 snapshot smoke DONE: $OUT_BASE"
