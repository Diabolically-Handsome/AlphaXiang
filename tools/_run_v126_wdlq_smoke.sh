#!/bin/bash
# v12.6-micro WDL-Q smoke test.
# Uses WDL expectation P(win)-P(loss) as the value scalar consumed by MCTS.
...
OUT_BASE="/home/laure/alphaxiang/v126_wdlq_smoke"
...
queue_cuda0() {
    ...
    run_external "pika_d3" 3 cuda:0 63003
    echo "[cuda:0] done $(date +%H:%M:%S)"
}

queue_cuda1() {
    echo "[cuda:1] start $(date +%H:%M:%S)"
    run_external "pika_d4" 4 cuda:1 63004
...
wait $PID1 || STATUS=$?
if [ "$STATUS" -ne 0 ]; then
    echo "v12.6-micro WDL-Q smoke FAILED with status=$STATUS" >&2
    exit "$STATUS"
fi
...
echo "v12.6-micro WDL-Q smoke DONE: $OUT_BASE"
