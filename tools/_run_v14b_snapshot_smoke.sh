...
OUT_ROOT="/home/laure/alphaxiang/v14b_snapshot_smoke/${TAG}"
...
run_arena() {
  ...
  "$PY" -u tools/external_arena.py \
    ...
    --seed "$((202605120 + depth * 100 + games))" \
    --device "$DEVICE" \
    --opp-engine pikafish \
    --opp-depth "$depth" \
    --opp-threads 1 \
...
# First gate: do not spend d6 unless these low-depth cells move.
for depth in 3 4 5; do
  run_arena "pika_d${depth}_g6" "$depth" 6 alternate model_only
done

run_arena "pika_d5_g6" 5 6 alternate ship \
