#!/bin/bash
# v12.8F: global strategic-attention adapter, but allow the last two
# Transformer blocks to adapt under v12.6 behavior anchoring.
...
OUT_DIR="${V128_OUT_DIR:-/home/laure/alphaxiang/training_runs/run_019g_v128_global_strategy_last2_anchor}"
DEVICE="${V128_TRAIN_DEVICE:-cuda:0}"
MAX_STEPS="${V128_MAX_STEPS:-297000}"
LR="${V128_LR:-2e-5}"
MICRO_BATCH="${V128_MICRO_BATCH:-96}"
STRATEGY_TOKENS="${V128_GLOBAL_STRATEGY_TOKENS:-6}"
ANCHOR_POLICY_KL="${V128_ANCHOR_POLICY_KL:-0.08}"
ANCHOR_VALUE_MSE="${V128_ANCHOR_VALUE_MSE:-0.03}"
UNFREEZE_LAST_N="${V128_UNFREEZE_LAST_N_BLOCKS:-2}"
...
"$PY" xiangqi_train.py \
    ...
    --adapter-unfreeze-last-n-blocks "$UNFREEZE_LAST_N" \
