#!/bin/bash
# v12.6-micro symbolic tactical-extension smoke test.
# Enables a conservative mate-in-1 leaf extension inside C++ MCTS.
...
OUT_BASE="/home/laure/alphaxiang/v126_tactical_mate1_smoke"
...
queue_cuda0() {
    ...
    run_external "pika_d3" 3 cuda:0 64003
    echo "[cuda:0] done $(date +%H:%M:%S)"
}

queue_cuda1() {
    echo "[cuda:1] start $(date +%H:%M:%S)"
    run_external "pika_d4" 4 cuda:1 64004
...
wait $PID1 || STATUS=$?
if [ "$STATUS" -ne 0 ]; then
    echo "v12.6-micro tactical mate-1 smoke FAILED with status=$STATUS" >&2
    exit "$STATUS"
fi
...
echo "v12.6-micro tactical mate-1 smoke DONE: $OUT_BASE"
